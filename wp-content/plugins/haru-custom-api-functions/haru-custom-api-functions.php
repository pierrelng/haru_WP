...
function haru_register_routes() {
  ...
  register_rest_route( 'haru/v1', 'venues/(?P<id>\d+)', array(
    'methods'  => WP_REST_Server::READABLE,
...
function haru_get_specific_venue( WP_REST_Request $request ) {
  ...
  if ($post) {
    $controller = new WP_REST_Posts_Controller('venues');
    $data = $controller->prepare_item_for_response( $post, $request );
    $event = $controller->prepare_response_for_collection( $data );
    if ($event['type'] === 'venues' && $event['status'] === 'publish') {
      return new WP_REST_Response($event, 200);
    } else {
